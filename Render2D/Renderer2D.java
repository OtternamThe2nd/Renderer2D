...
Variables
	...
	(private int)		height		:	holds the height of the canvas measured on pixels
...
public class Renderer2D {
	...
	public void drawCircle(int x, int y, int radius,byte color[]){
		for(int yorigin=-radius;yorigin<=radius;yorigin++) {
			...
			for(int xstart=-xorigin;xstart<=xorigin;xstart++) {
				int x2=x+xstart;
				int y2=y+yorigin;
				if(x2<0||y2<0||y2>=canvas.length||x2>=canvas[0].length/3) continue;
				canvas[y2][x2*3]=color[2];
				canvas[y2][x2*3+1]=color[1];
				canvas[y2][x2*3+2]=color[0];
	...
	public void drawTriangle(int xpoints[],int ypoints[],byte[] color) {
		...
		for(int y2=yorigin+1;y2<xorigins.length+yorigin;y2++) {
